...
namespace System
{
    internal sealed class PinnableBufferCache
    {
        ...
        /// </summary>
        public byte[] AllocateBuffer() => (byte[])Allocate();
        ...
        internal object Allocate()
        {
            ...
            if (!_freeList.TryPop(out var returnBuffer))
            {
            ...
            {
                var numAllocCalls = Interlocked.Increment(ref _numAllocCalls);
                if (numAllocCalls >= 1024)
                {
                    lock (this)
                    {
                        var previousNumAllocCalls = Interlocked.Exchange(ref _numAllocCalls, 0);
                        if (previousNumAllocCalls >= 1024)
                        {
                            var nonGen2Count = 0;
                            foreach (var o in _freeList)
                            {
        ...
        private void CreateNewBuffers()
        {
            ...
            for (var i = 0; i < _restockSize; i++)
            {
                // Make a new buffer.
                var newBuffer = _factory();
        ...
        /// </summary>
        [System.Security.SecuritySafeCritical]
        private static bool Gen2GcCallbackFunc(object targetObj) => ((PinnableBufferCache)(targetObj)).TrimFreeListIfNeeded();
        ...
        {
            var curMSec = Environment.TickCount;
            var deltaMSec = curMSec - _msecNoUseBeyondFreeListSinceThisTime;
            ...
            lock (this)
            {
                ...
                {
                    if (_freeList.TryPop(out var buffer))
                    {
                        _notGen2.Add(buffer);
                    }
                }
    ...
    internal sealed class Gen2GcCallback //: CriticalFinalizerObject
    {
        ...
        public static void Register(Func<object, bool> callback, object targetObj)
        {
            ...
            var gcCallback = new Gen2GcCallback();
