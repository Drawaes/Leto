...
namespace Leto.Windows.Interop
{
    internal partial class BCrypt
    {
        ...
        internal static unsafe int BCryptDecrypt(SafeBCryptKeyHandle key, Span<byte> inputOutput, BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO* info, void* ivBuffer)
        {
        ...
        internal static unsafe int BCryptDecrypt(SafeBCryptKeyHandle key, Span<byte> input, Span<byte> output, BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO* info, void* ivBuffer)
        {
            fixed(void* inPtr = &input.DangerousGetPinnableReference())
            fixed(void* outPtr = &output.DangerousGetPinnableReference())
            {
                var result = BCryptDecrypt(key, inPtr, input.Length, info, ivBuffer, info[0].cbMacContext, outPtr, output.Length, out int bytesWritten, 0);
                return bytesWritten;
            }
        }
