...
namespace SocketServer
{
    public class RequestHeaderDictionary : IHeaderDictionary
    {
        ...
        {
            var headerKey = GetHeaderKey(ref key);
        ...
        private void SetHeader(string key, StringValues value) => _headers[key] = new HeaderValue
        {
            Value = value
        };

        public void Add(KeyValuePair<string, StringValues> item) => SetHeader(item.Key, item.Value);

        public void Add(string key, StringValues value) => SetHeader(key, value);
        ...
        public void CopyTo(KeyValuePair<string, StringValues>[] array, int arrayIndex) => throw new NotSupportedException();
        ...
        public IEnumerator<KeyValuePair<string, StringValues>> GetEnumerator() => _headers.Select(h => new KeyValuePair<string, StringValues>(h.Key, h.Value.GetValue())).GetEnumerator();
